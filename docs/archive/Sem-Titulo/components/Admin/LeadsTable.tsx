...
export function LeadsTable({ leads, onRefresh }: LeadsTableProps) {
  ...
  const filteredLeads = leads.filter((lead) => {
    ...
    const matchesRevenue = filterRevenue === 'all' || lead.revenue_range === filterRevenue
        ...
        <table className="w-full">
          ...
          <tbody className="bg-white divide-y divide-elite-navy/10">
            {filteredLeads.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-8 text-center text-elite-navy/50">
                  Nenhum lead encontrado
                </td>
              </tr>
            ) : (
              filteredLeads.map((lead) => (
                <tr key={lead.id} className="hover:bg-elite-quartz/50 transition-colors">
