  ...
  const onSubmit = async (data: LeadFormSchema) => {
    ...
    try {
      ...
      if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
        // Se não estiver configurado, apenas redireciona (modo demo)
    ...
    <div className="bg-white rounded-2xl p-8 md:p-10 shadow-xl border border-elite-navy/10">
      <div className="mb-8">
        ...
        <p className="text-elite-navy/70">
          Preencha o formulário abaixo para garantir sua participação no meet exclusivo
        </p>
      </div>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div>
          <label htmlFor="nome" className="block text-sm font-medium text-elite-navy mb-2">
            Nome completo *
          </label>
          <input
            {...register('nome')}
        ...
        <div>
          <label htmlFor="email" className="block text-sm font-medium text-elite-navy mb-2">
            Email corporativo *
          </label>
          <input
            {...register('email')}
        ...
        <div>
          <label htmlFor="whatsapp" className="block text-sm font-medium text-elite-navy mb-2">
            WhatsApp com DDD *
          </label>
          <WhatsAppInput
            value={whatsappValue}
        ...
        <div>
          <label htmlFor="revenue_range" className="block text-sm font-medium text-elite-navy mb-2">
          ...
          {errors.revenue_range && (
            <p className="mt-1 text-sm text-red-500">{errors.revenue_range.message}</p>
          )}
        </div>
        ...
        <p className="text-xs text-elite-navy/60 text-center">
          Ao preencher este formulário, você concorda em receber informações sobre o webinar.
        </p>
      </form>
    </div>
  )
